...
use AppBundle\Exceptions\PostException;
...
class PhotoService
{
    ...
    public function upload(UploadedFile $file, $targetDir, $convertToJPG = false, $type = null)
    {
        $filePath = '';
        try {
           if ($type) {
               $fullPath = $this->webDir.'/'.$this->baseDir.'/'.$targetDir.'/'.$type;

               $format = $file->getClientOriginalExtension();
               $fileName = uniqid().'.'.$format;
               $file->move($fullPath, $fileName);
               $filePath = $this->baseDir.'/'.$targetDir.'/'.$type.'/'.$fileName;
